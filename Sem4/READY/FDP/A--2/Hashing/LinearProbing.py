...
class hashTable:
    ...
    def insert(self, record):
        ...
        position = self.hashFunction(record.get_number())         #getting the position (index)
       
        # checking if that position is empty
    ...
    def search(self, record):
        ...
        if(self.table[position] != None):         #if something is stored at that location then..
        ...
        # if element is not found at position returned by hash function then..
        
            else:
                position += 1
                if position >= self.size-1:
                    position = 0
                while self.table[position] != 0 or self.comparisons <= self.size:
